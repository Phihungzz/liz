...
const BackgroundControls = ({ project }) => {
    ...
    const searchUnsplashImages = async () => {
        ...
        try {
            ...
            if (!response.ok) toast.error("Failed to search image");
            const data = await response.json();
            setUnsplashImages(data.results || []);

        } catch (error) {
            console.error("Error searching Unsplash:",error);
            toast.error("Failed to search image. Please try again");
        } finally {
    ...
    const handleImageBackground = async (imageUrl, imageId) => {
        ...
        try {
            if (UNSPLASH_ACCESS_KEY) {
                fetch(`${UNSPLASH_API_URL}/photo/${imageId}/download`, {
                    headers: {
                        Authorization: `Client-ID ${UNSPLASH_ACCESS_KEY}`,
                    },
                }).catch(() => {});
            ...
            fabricImage.set({
                scaleX: scale,
                scaleY: scale,
                originX: "center", //center hori
                originY: "center", //center verti
            ...
            <div>
                <div>
                    ...
                    <p className="text-xs text-white/70 mb-4">
                        Automatically remove background from your image using AI
                    </p>
                </div>
                ...
                {!getMainImage() && (
                    <p className="text-xs text-amber-400">
                        Please add image to the canvas first to remove background
                    </p>
                )}
            </div>
            ...
            <Tabs defaultValue="color" className="w-full">
                ...
                <TabsContent value="image" className="space-y-4 mt-6">
                    <div>
                        ...
                        <p className="text-xs text-white/70 mb-4">
                            Search and use high-quality image
                        </p>
                    </div>
                    ...
                    <div className="flex gap-2">
                        <Input 
                            ...
                            onKeyPress={handleSearchKeyPress} // alow enter to search
                            ...
                            <div className="grid grid-cols-2 gap-3 max-h-96 overflow-y-auto">
                                {unsplashImages.map(image => {
                                    return (
                                        <div
                                            key={image.id}
                                            className="relative group cursor-pointer rounded-lg overflow-hidden border botder-white/10 hover:border-cyan-400 transition-colors"
                                            onClick={() => 
                                        ...
                                        >
                                            ...
                                            <div className="absolute inset-0 bg-black/0 group-hover:bg-black/30 transiton-colors flex items-center justify-center opacity-0 group-hover:opacity-100">
